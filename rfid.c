...
#define pinHigh GPIO_NUM_17

#define CONFIG_FREERTOS_HZ 100

static rc522_handle_t scanner;

static const char* TAG = "NIK & JOAOP";
...
{
     // Aloca memória para um buffer para armazenar o endereço e os dados a serem escritos

    uint8_t* buffer = (uint8_t*) malloc(n + 1); // FIXME: memcheck
    buffer[0] = addr;                // Define o primeiro byte do buffer como o endereço
    memcpy(buffer + 1, data, n);     // Copia os dados a serem escritos no buffer após o endereco
    esp_err_t ret;                  // Variável para armazenar o valor de retorno
    switch(rc522->config->transport) {                      // Verifica o tipo de transporte do dispositivo RC522
        case RC522_TRANSPORT_SPI:           // Se for SPI
            ret = rc522_spi_send(rc522, buffer, n + 1);          // Envia os dados via SPI
            break;
        case RC522_TRANSPORT_I2C:           // Se for I2C
            ret = rc522_i2c_send(rc522, buffer, n + 1);          // Envia os dados via I2C
            break;
        default:                     // Se for um tipo de transporte desconhecido
            ESP_LOGE(TAG, "gravou: Transporte Desconhecido");          // Registra uma mensagem de erro
            ret = ESP_ERR_INVALID_STATE; // Define o valor de retorno como um código de erro para transporte desconhecido
    }
    free(buffer);                                // Libera a memória alocada para o buffer
    if(ESP_OK != ret) {                         // Se houve um erro ao enviar os dados
        ESP_LOGE(TAG, "Falha ao gravar dados (err: %s)", esp_err_to_name(ret));         // Registra uma mensagem de erro com detalhes sobre o erro
    }
    return ret;                                 // Retorna o resultado da operação (ESP_OK se bem-sucedido, um código de erro caso contrário)
}
...
static uint8_t* rc522_read_n(rc522_handle_t rc522, uint8_t addr, uint8_t n)
{
    ...
    switch(rc522->config->transport) {
        ...
        case RC522_TRANSPORT_I2C:
            ...
            break;
        default:
            ESP_LOGE(TAG, "leitura: Transporte desconhecido");
            ret = ESP_ERR_INVALID_STATE; // unknown transport
    }
    if(ESP_OK != ret) {
        free(buffer);
        buffer = NULL;
        ESP_LOGE(TAG, "Falha ao ler os dados (err: %s)", esp_err_to_name(ret));
    }
    return buffer;
}
...
static esp_err_t rc522_create_transport(rc522_handle_t rc522)
{
    ...
    switch(rc522->config->transport) {
        ...
        case RC522_TRANSPORT_I2C: {
            ...
            break;
        default:
            ESP_LOGE(TAG, "create_transport: Transporte desconhecido");
            ret = ESP_ERR_INVALID_STATE; // unknown transport
            break;
    }
...
esp_err_t rc522_create(rc522_config_t* config, rc522_handle_t* out_rc522)
{
    ...
    if(ESP_OK != (ret = rc522_create_transport(rc522))) {
        ESP_LOGE(TAG, "Não é possível criar transporte");
        rc522_destroy(rc522);
        return ret;
    }
    ...
    if(ESP_OK != (ret = esp_event_loop_create(&event_args, &rc522->event_handle))) {
        ESP_LOGE(TAG, "Não é possível criar loop de evento");
        rc522_destroy(rc522);
        return ret;
    }
    ...
    if (xTaskCreate(rc522_task, "rc522_task", rc522->config->task_stack_size, rc522, rc522->config->task_priority, &rc522->task_handle) != pdTRUE) {
        ESP_LOGE(TAG, "Não é possível criar tarefa");
        rc522_destroy(rc522);
        return ret;
    }
    ...
    if(! rc522->initialized) {
        ...
        for(uint8_t i = test_val; i < test_val + 2; i++) {
            if((err = rc522_write(rc522, test_addr, i)) != ESP_OK || rc522_read(rc522, test_addr) != i) {
                ESP_LOGE(TAG, "Falha no teste de leitura/gravação");
                rc522_destroy(rc522);
                return err;
            }
        }
        ...
        ESP_LOGI(TAG, "Iniciando: (firmware v%d.0)", (rc522_firmware(rc522) & 0x03));
    }
...
static void rc522_destroy_transport(rc522_handle_t rc522)
{
    switch(rc522->config->transport) {
        ...
        case RC522_TRANSPORT_I2C:
            ...
            break;
        default:
            ESP_LOGW(TAG, "destroy_transport: Transporte desconhecido");
    }
}
...
void rc522_destroy(rc522_handle_t rc522)
{
    ...
    if(xTaskGetCurrentTaskHandle() == rc522->task_handle) {
        ESP_LOGE(TAG, "Não é possível destruir rc522 do manipulador de eventos");
        return;
    }
...
static void rc522_handler(void* arg, esp_event_base_t base, int32_t event_id, void* event_data)
{
    ...
    switch(event_id) {
        case RC522_EVENT_TAG_SCANNED: {
                ...
                ESP_LOGI(TAG, "Tag digitalizada (sn: %" PRIu64 ")", tag->serial_number);
            }
            break;
    }
}
...
// static void tag_scanned_handler(void* handler_arg, esp_event_base_t base, int32_t id, void* event_data)
// {
//  rc522_tag_t* tag = (rc522_tag_t*) event_data;
//                 printf("Tag digitalizada: %llu\n", tag->serial_number);
// }
